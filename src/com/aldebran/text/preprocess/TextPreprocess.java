...
public class TextPreprocess implements Serializable {
...

    {
        ...
        // （3）数字替换
        replaceInfos.add(new ReplaceInfo(Constants.NUMBER_REPLACEMENT, Pattern.compile("\\d+(\\.\\d+)?"), Pattern.compile("\\d+(\\.\\d+)?")));
    ...
    public List<String> textToUnits(String preprocessText) {
        ...
        for (String s : sp) {
            ...
            if (matcher.find()) {
                ...
                int ed = matcher.end();
                if (st == 0) {
                    String word = englishRoot(s.substring(st, ed));
                    result.add(word);
                    ...
                    String word = englishRoot(s.substring(st));
                    result.add(word);
                }
            } else {
    ...
    //  TODO 英文可追加词根识别
    public String englishRoot(String word) {
        return word;
    }
